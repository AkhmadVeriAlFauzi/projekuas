                ...
                <div class="card-body">
                    <form action="{{'/'.$criteria->id.'/editKriteria'}}" method="post">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="kode_kriteria" value="{{ $criteria->kode_kriteria }}" required>
                            @error('kode_kriteria') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-3">
                            <label for="nama_kriteria" class="form-label">Nama Kriteria</label>
                            <input type="text" class="form-control" name="nama_kriteria" value="{{ $criteria->nama_kriteria }}" required>
                            @error('nama_kriteria') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-3">
                            <label for="cb" class="form-label">Cost/Benefit</label>
                            <select class="form-select" name="cb" value="{{ $criteria->cb }}"required>
